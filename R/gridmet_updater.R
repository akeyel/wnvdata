...
process.month = function(rda.path, rda.april, rda.may, rda.june, rda.processed, weather.files, april.files, may.files, june.files, breaks, i){
  ...
  # Join May data to Most Data
  rm(this.state)
  load(sprintf("%s/%s", rda.may, may.files[i]))
  state.may = substr(may.files[i], 1, nchar(may.files[i]) - 10)
  this.state$location_year = sprintf("%s-%s_%s", state.may, this.state$district, this.state$year)
  this.state$location = sprintf("%s-%s", state.may, this.state$district)
